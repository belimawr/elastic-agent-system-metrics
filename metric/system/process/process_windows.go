...
func GetInfoForPid(_ resolve.Resolver, pid int) (ProcState, error) {
	...
	// fmt.Println("handle, err := syscall.OpenProcess(windows.PROCESS_QUERY_LIMITED_INFORMATION, true, uint32(pid)), err:", err)
	if err != nil {
	...
	// fmt.Println("handle, err := syscall.OpenProcess(windows.PROCESS_QUERY_LIMITED_INFORMATION, false, uint32(pid)), err:", err)
	if err != nil {
...
func FetchNumThreads(pid int) (int, error) {
	// First fix. Use PROCESS_QUERY_LIMITED_INFORMATION
	pHandle, err := syscall.OpenProcess(windows.PROCESS_QUERY_LIMITED_INFORMATION, false, uint32(pid))
	// fmt.Println("pHandle, err := syscall.OpenProcess(windows.PROCESS_QUERY_LIMITED_INFORMATION, false, uint32(pid)), err:", err)
	if err != nil {
	...
	// Below is the autogenerated function without the error conversion
	_, _, err = syscall.Syscall6(
		procPssCaptureSnapshot.Addr(),
		4,
		uintptr(pHandle),
		uintptr(PSSCaptureThreads),
		uintptr(0),
		uintptr(unsafe.Pointer(&snapshotHandle)),
		0,
		0)
	// fmt.Println("err = PssCaptureSnapshot(pHandle, PSSCaptureThreads, 0, &snapshotHandle), err: <nil>")
	// golang.org/x/sys/windows.ERROR_SUCCESS
	// Function documentation https://learn.microsoft.com/en-us/windows/win32/api/processsnapshot/nf-processsnapshot-psscapturesnapshot
	// Error codes documentation: https://learn.microsoft.com/en-us/windows/win32/debug/system-error-codes--0-499-
	if err != xsyswindows.ERROR_SUCCESS {
		// fmt.Println("err = PssCaptureSnapshot(pHandle, PSSCaptureThreads, 0, &snapshotHandle), err:", err)
...
func getProcName(pid int) (string, error) {
	handle, err := syscall.OpenProcess(windows.PROCESS_QUERY_LIMITED_INFORMATION, true, uint32(pid))
	if err != nil {
